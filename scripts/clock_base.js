...
function MartianTime(){
	...
	// delete following line, Mars24J v8.3.1 is wrong!
    ...
    // 0.00036 equals 31.104 seconds, so we need to adjust mtc too
    mtc = (mtc * 3600)-31;  // convert from hours to seconds, apply the fudge
    mtc = Math.floor(mtc);
	...
	zyear = Math.floor(meridian / martian_year)+1;
	zmonth = Math.floor(parseInt(lSubS)/30)+1;
	zdays = Math.floor(((meridian % martian_year) / martian_day))+1;

	mission_count = (parseInt(mmsd.toFixed()) - piri.msd_offset)+1;

    xxtc = Math.floor(mtc + calcOffset(piri.actual_lon)) % 86400;
    //conslog(mtc);
    //conslog(xxtc);
    if( xxtc < mtc && piri.actual_lon < 180 ){
    	zdays++;
    	mission_count++;
    	if( zdays > 668 ){
    		zyear++;
    ...
    else if( xxtc > mtc && piri.actual_lon > 180 ){
		zdays--;
		mission_count--;
    	if( zdays == 0 ){
    		zyear--;
...
function calcMiddayAndDayLen(jd_tt, piri, l_s){
